...
#pragma warning disable IDE0060 // Remove unused parameter
        ...
        private void EmitSpace(IEmitter emitter, ISpace ispace)
        {
            ...
            emitter.EmitKVP("Terrain", space.Terrain.Name);  //inprog

            var kst = FlagString(space.IsKnown, space.IsSown, space.IsTilled);
            emitter.EmitKVP("Flags", kst);
        }

        private void EmitTerrain(IEmitter emitter, Terrain terrain)
        {
            //terrain.
            //emitter.EmitKVP("ID", terrain.ID.ToString(CultureInfo.InvariantCulture));
            //emitter.EmitKVP("Terrain", space.Terrain.Name);  //inprog
