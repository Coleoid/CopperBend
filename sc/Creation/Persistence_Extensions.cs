...
namespace CopperBend.Creation
{
    public static class Persistence_Extensions
    {
        ...
        {
            parser.GetScalar(valueName);
            return parser.GetScalar();
        }
        ...
        public static string GetScalar(this IParser parser, string expected)
        {
            parser.TryConsume<Scalar>(out Scalar scalar);
            if (scalar.Value != expected)
                throw new Exception($"Expected '{expected}', got '{scalar.Value}'.");
            return scalar.Value;
        }
